...
router.get('/dish/side', async function (req, res, next) {
    let queryresult = await db.connection.queryAsync(
        "SELECT * FROM `dish` WHERE type = '附餐'"
    );
    res.send(queryresult);
});
...
router.get('/dish/dessert', async function (req, res, next) {
    let queryresult = await db.connection.queryAsync(
        "SELECT * FROM `dish` WHERE type = '甜點'"
    );
    res.send(queryresult);
});
...
router.get('/coupon/:memberId', async function (req, res, next) {
    ...
    let memberCoupon = await db.connection.queryAsync(queryresult, [
        req.params.memberId,
    ]);
...
router.get('/member/:memberId', async function (req, res, next) {
    let queryresult =
        'SELECT m.member_id ,m.name,m.mobile FROM member m WHERE m.member_id = ?';
    let memberInfo = await db.connection.queryAsync(queryresult, [
        req.params.memberId,
    ]);
    // console.log('memberInfo:', queryresult)
    res.send(memberInfo);
});
...
router.post('/order', async function (req, res, next) {
    ...
    // 餐點詳情
    sql =
        'INSERT INTO delivery (name,mobile,address,delivery_time,total,note,member_id,mcm_id) VALUES (?)';
    ...
    let updateMcm = await db.connection.queryAsync(McmSql, [
        req.body.data.mcm_id,
    ]);
    ...
    for (let i = 0; i < list.length; i++) {
        for (let j = 0; j < list[i][1]; j++) {
            newDishList.push({
                delivery_id: updateDeliery.insertId,
                dish_id: list[i][2],
            });
        }
    }
    ...
    for (let i = 0; i < newDishList.length; i++) {
        let insertDish = await db.connection.queryAsync(
            `INSERT INTO delivery_dish_mapping (delivery_id, dish_id) VALUES (?,?)`,
            [[newDishList[i].delivery_id], [newDishList[i].dish_id]]
        );
    }
